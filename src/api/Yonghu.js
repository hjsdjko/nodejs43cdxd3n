...
export default ({ config, db }) => {
	...
	api.get('/count', async (req, res) => {
		...
		try {
			let where = {}
			var yonghuzhanghao = req.query.yonghuzhanghao
			if (yonghuzhanghao) {
			...
			var yonghuxingming = req.query.yonghuxingming
			if (yonghuxingming) {
	...
	api.get('/sectionStat/nianling', async (req, res) => {
		...
		try {
			...
			sql = `
				...
				from
				(select
				sum(case when nianling >= 0 and nianling <= 17 then 1 else 0 end) as 青少年,				sum(case when nianling >= 18 and nianling <= 45 then 1 else 0 end) as 青年,				sum(case when nianling >= 46 and nianling <= 69 then 1 else 0 end) as 中年,				sum(case when nianling >= 70 then 1 else 0 end) as 老年				from yonghu
				${where}
        		) t
        		union all
